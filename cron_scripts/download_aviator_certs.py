...
Ths script scans the Google Aviator CT database for certificates related to Marinus SSL_Orgs.
...
Data is written to: /mnt/workspace/ct_aviator/

The certificate transparency github project must be installed in /mnt/workspace/certificate-transparency/
The certificate transparency github project is located at: https://github.com/google/certificate-transparency/
"""
...
gflags.DEFINE_string("output", "/mnt/workspace/ct_aviator/",
...
def write_matched_certificate(matcher_output):
    """
    Write matched certificate to the file system.
    """
...
def run():
    """
    Execute the scanner in multi-threaded mode.
    """
    if not FLAGS.output:
        raise Exception("Certificates output directory must be specified.")

    res = scanner.scan_log(
        match, "https://ct.googleapis.com/aviator", FLAGS.multi,
