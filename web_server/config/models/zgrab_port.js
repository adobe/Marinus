            ...
            if (count) {
                return zPortSchema.zgrabPortModel.find({
                    '$or': [{'domains': domain},
                            ...
                            {'data.smtp.tls.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}).countDocuments().exec();
            } else {
                return zPortSchema.zgrabPortModel.find({
                    '$or': [{'domains': domain},
                            {'data.smtp.tls.server_certificates.certificate.parsed.subject.common_name': domain},
                            {'data.smtp.tls.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}, {'ip': 1, 'data.smtp': 1}).exec();
            }
        } else if (port === "443") {
            if (count) {
                return zPortSchema.zgrabPortModel.find({
                    '$or': [{'domains': domain},
                            ...
                            {'data.tls.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}).countDocuments().exec();
            } else {
                return zPortSchema.zgrabPortModel.find({
                    '$or': [{'domains': domain},
                            {'data.tls.server_certificates.certificate.parsed.subject.common_name': domain},
                            {'data.tls.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}, {'ip': 1, 'data.tls': 1}).exec();
            }
        } else if (port === "465") {
            if (count) {
                return zPortSchema.zgrabPortModel.find({
                    '$or': [{'domains': domain},
                            ...
                            {'data.smtps.tls.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}).countDocuments().exec();
            } else {
                return zPortSchema.zgrabPortModel.find({
                    '$or': [{'domains': domain},
                            {'data.smtps.tls.server_certificates.certificate.parsed.subject.common_name': domain},
                            {'data.smtps.tls.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}, {'ip': 1, 'data.smtps': 1}).exec();
            }
        } else if (count) {
            return(zPortSchema.zgrabPortModel.find({'domains': domain}).countDocuments().exec());
        }
