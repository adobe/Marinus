            ...
            if (count) {
                return z2PortSchema.zgrab2PortModel.find({
                    '$or': [{'domains': domain},
                            ...
                            {'data.smtp.result.tls.handshake_log.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}).countDocuments().exec();
            } else {
                return z2PortSchema.zgrab2PortModel.find({
                    '$or': [{'domains': domain},
                            {'data.smtp.result.tls.handshake_log.server_certificates.certificate.parsed.subject.common_name': domain},
                            {'data.smtp.result.tls.handshake_log.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}, {'ip': 1, 'data.smtp': 1}).exec();
            }
        } else if (port === "443") {
            if (count) {
                return z2PortSchema.zgrab2PortModel.find({
                    '$or': [{'domains': domain},
                            ...
                            {'data.tls.result.handshake_log.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}).countDocuments().exec();
            } else {
                return z2PortSchema.zgrab2PortModel.find({
                    '$or': [{'domains': domain},
                            {'data.tls.result.handshake_log.server_certificates.certificate.parsed.subject.common_name': domain},
                            {'data.tls.result.handshake_log.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}, {'ip': 1, 'data.tls': 1}).exec();
            }
        } else if (port === "465") {
            if (count) {
                return z2PortSchema.zgrab2PortModel.find({
                    '$or': [{'domains': domain},
                            ...
                            {'data.smtps.result.tls.handshake_log.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}).countDocuments().exec();
            } else {
                return z2PortSchema.zgrab2PortModel.find({
                    '$or': [{'domains': domain},
                            {'data.smtps.result.tls.handshake_log.server_certificates.certificate.parsed.subject.common_name': domain},
                            {'data.smtps.result.tls.handshake_log.server_certificates.certificate.parsed.extensions.subject_alt_name.dns_names': domain}]}, {'ip': 1, 'data.smtps': 1}).exec();
            }
        } else if (count) {
            return(z2PortSchema.zgrab2PortModel.find({'domains': domain}).countDocuments().exec());
        }
