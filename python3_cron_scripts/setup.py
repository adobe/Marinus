...
def create_job_collection(mongo_connector):
    ...
    script_names = ["remote_download", "get_virustotal_data", "get_riskiq_data", "get_iblox_cname", "get_iblox_host", "get_iblox_alpha_zones",
                    "get_aws_data", "send_remote_server", "remove_expired_entries", "get_iblox_a", "get_data_by_cidr_dns", "get_crt_sh",
...
def create_user(mongo_connector, username):
    ...
    randomValue = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(32)])

    user_collection.insert({"userid": username, "status": "active", "created": now, "apiKey": randomValue})
...
def create_first_groups(mongo_connector, username):
    ...
    group_collection.insert({"name": "admin", "status": "active", "creation_date": now, "updated": now, "creator": username, "members": members, "admins": admins})
    group_collection.insert({"name": "data_admin", "status": "active", "creation_date": now, "updated": now, "creator": username, "members": members, "admins": admins})
...
def add_admin_to_group(mongo_connector, username, group):
    ...
    group_collection.update({"name": group}, {"$addToSet": {"admins": username}, "$set": {"updated": now}})
...
def add_tls_org(mongo_connector, org):
    ...
    config_collection.update({}, {"$addToSet": {"SSL_Orgs": org}, "$set": {"updated": now}})
...
def add_dns_admin(mongo_connector, dns_admin):
    ...
    config_collection.update({}, {"$addToSet": {"DNS_Admins": dns_admin}, "$set": {"updated": now}})
