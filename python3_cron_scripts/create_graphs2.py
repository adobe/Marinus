        ...
        if result['type'] == "a":
            ...
            if str(result['fqdn']) != zone:
                graph.add_node(result['fqdn'].replace("." + zone, ""),
                               ...
                               dependedOnBy=[result['value']], docs="")
            else:
                graph.add_node(result['fqdn'], data_type="tld",
                               type=zone_g_index, depends=[],
                               dependedOnBy=[result['value']], docs="")
            graph.add_node(result['value'], data_type="ip", type=ip_g_index,
                           depends=[result['fqdn'].replace("." + zone, "")],
                           dependedOnBy=[], docs="")
            graph.add_edge(zone, result['fqdn'].replace("." + zone, ""), value=2)
            graph.add_edge(result['fqdn'].replace("." + zone, ""), result['value'], value=1)
            ...
            if result['fqdn'] != zone:
                graph.add_node(result['fqdn'].replace("." + zone, ""),
                               data_type="domain", type=zone_g_index, depends=[zone],
                               dependedOnBy=[result['value'].replace("." + zone, "")], docs="")
            else:
                graph.add_node(result['fqdn'], data_type="tld", type=zone_g_index, depends=[],
                               dependedOnBy=[result['value'].replace("." + zone, "")], docs="")
            graph.add_node(result['value'].replace("." + zone, ""), data_type="cname",
                           type=cname_g_index, depends=[result['fqdn'].replace("." + zone, "")],
                           dependedOnBy=[], docs="")
            graph.add_edge(zone, result['fqdn'].replace("." + zone, ""), value=2)
            graph.add_edge(result['fqdn'].replace("." + zone, ""),
    ...
    for result in rdns_results:
        ...
        if result['fqdn'] != zone:
            graph.add_node(result['fqdn'].replace("." + zone, ""), data_type="domain",
                           type=zone_g_index, depends=[zone], dependedOnBy=[result['ip']], docs="")
        else:
            graph.add_node(result['fqdn'], data_type="tld", type=zone_g_index, depends=[],
    ...
    if node['data_type'] == "ip":
        ...
        if groups[node['type']].replace(REPLACE_CHAR, ".") == zone:
            html += ("<a href=\"/domain?search=" + node['id'] + "." +
                     zone + "\" target=\"_blank\">Link to full host details</a>")
        else:
            html += ("<a href=\"/domain?search=" + node['id'] +
...
def main():
    ...
    for zone in zones:
        ...
        new_graph_data['data'] = {}
        for i in range(0, len(data['nodes'])):
            new_graph_data['data'][data['nodes'][i]['id'].replace(".", REPLACE_CHAR)] = data['nodes'][i]
            new_docs_data['docs'][data['nodes'][i]['id'].replace(".", REPLACE_CHAR)] = data['nodes'][i]['docs']
            del new_graph_data['data'][data['nodes'][i]['id'].replace(".", REPLACE_CHAR)]['docs']
