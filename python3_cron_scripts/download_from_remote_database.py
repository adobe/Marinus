...
def download_zgrab_info(logger, zgrab_data_collection, remote_zgrab_data_collection):
    ...
    # Grab the new results from the remote server.
    ip_results = remote_zgrab_data_collection.find({'domain': "<nil>", 'timestamp': {"$gte": yesterday}}, {"_id": 0}).batch_size(50)
    ...
    # Grab the new results from the remote server.
    domain_results = remote_zgrab_data_collection.find({'ip': "<nil>", 'timestamp': {"$gte": yesterday}}, {"_id": 0}).batch_size(50)
...
def download_zgrab_port_info(logger, zgrab_data_collection, remote_zgrab_data_collection):
    ...
    # Grab the new results from the remote server.
    ip_results = remote_zgrab_data_collection.find({'timestamp': {"$gte": yesterday}}, {"_id": 0}).batch_size(50)
...
def download_amass_data(logger, amass_collection, remote_amass_collection, dns_manager, zones):
    """
    ...
    amass_results = remote_amass_collection.find({'timestamp': {"$gt": mirror_date}}, {"_id": 0}).batch_size(50)
    ...
    for result in amass_results:
        ...
        if zone is not None:
            ...
            if record_finding(logger, dns_manager, google_dns, zone, result):
                amass_collection.replace_one({'name': result['name']}, result, upsert=True)
...
def main():
    ...
    if status['status'] == jobs_manager.COMPLETE:
        ...
        remote_jobs_collection.update_one({'job_name': 'whois'}, {'$set': {'status': jobs_manager.READY}})
    ...
    download_amass_data(logger, amass_collection, remote_amass_collection, dns_manager, zones)
