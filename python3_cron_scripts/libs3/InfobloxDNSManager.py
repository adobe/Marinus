...
from libs3 import DNSManager, InfobloxHelper, MongoConnector, APIHelper
...
class InfobloxDNSManager(object):
    ...
    def __get_previous_records(self):
        ...
        """
        self.previous_records = self.iblox_collection.distinct('_ref', {'zone': self.zone_queried})
    ...
    def __insert_dns_information(self, dns_information):
        ...
        if self.record_type == 'host':
            ...
            dns_information['value'] = dns_information[self.dns_value_mapper[self.record_type]]
    ...
    def __backoff_api_retry(self):
        ...
        return requests.get((self.__get_record_type_url()),
                            auth=HTTPBasicAuth(self.IH.IBLOX_UNAME, self.IH.IBLOX_PASSWD), verify='/etc/ssl/certs/ca-certificates.crt')
