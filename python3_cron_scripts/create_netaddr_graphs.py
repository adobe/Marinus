...
def find_all_dns_by_zone(graph, ipzone, groups, dns_manager):
    ...
    dns_results = dns_manager.find_multiple({'type': "a", 'value': {'$regex': regx}}, None)

    for result in dns_results:
        zone = result['zone']
        zone_g_index = add_to_list(zone, groups)

        graph.add_node(result['value'], data_type="ip", type=0, depends=[ipzone],
                       dependedOnBy=[result['fqdn']], docs="")

        if str(result['fqdn']) != zone and zone != "":
            graph.add_node(zone, data_type="tld", type=zone_g_index,
                           depends=[result['fqdn'].replace("." + zone, "")],
                           dependedOnBy=[], docs="")
            graph.add_node(result['fqdn'].replace("." + zone, ""), data_type="domain",
                           type=zone_g_index, depends=[result['value']],
                           dependedOnBy=[zone], docs="")
        elif zone == "":
            graph.add_node(result['fqdn'], data_type="domain", type=zone_g_index, depends=[result['value']],
                           dependedOnBy=[], docs="")
        else:
            graph.add_node(zone, data_type="tld", type=zone_g_index, depends=[result['value']],
                           dependedOnBy=[], docs="")

        graph.add_edge(ipzone, result['value'], value=2)
        if zone != "":
            graph.add_edge(result['value'], result['fqdn'].replace("." + zone, ""), value=1)
    ...
    for result in rdns_results:
        ...
        if result['fqdn'] != zone and zone != "":
            graph.add_node(zone, data_type="tld", type=zone_g_index,
                           ...
                           dependedOnBy=[], docs="")
            graph.add_node(result['fqdn'].replace("." + zone, ""), data_type="domain",
                           type=zone_g_index, depends=[result['ip']],
                           dependedOnBy=[zone], docs="")
        elif zone == "":
            graph.add_node(result['fqdn'], data_type="domain", type=zone_g_index,
                           depends=[result['ip']], dependedOnBy=[], docs="")
        else:
            graph.add_node(result['fqdn'], data_type="tld", type=zone_g_index,
                           depends=[result['ip']], dependedOnBy=[], docs="")

        graph.add_node(result['ip'], data_type="ip", type=0, depends=[ipzone],
                       dependedOnBy=[result['fqdn'].replace("." + zone, "")], docs="")

        graph.add_edge(ipzone, result['ip'], value=2)
        if zone != "":
            graph.add_edge(result['ip'], result['fqdn'].replace("." + zone, ""), value=1)
...
def reformat_data(data, cidr, groups):
    ...
    for i in range(0, len(data['nodes'])):
        ...
        for tmp_links in data['links']:
            if data['nodes'][i]['id'] == tmp_links['target'] \
            ...
            if data['nodes'][i]['id'] == tmp_links['source'] \
...
def main():
    ...
    for tcidr in cidr_list:
        ...
        graph.add_node(cidr, data_type="class_c", type=0, depends=[],
